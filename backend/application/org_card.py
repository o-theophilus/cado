...
def approve(key):
    ...
    if (
        not request.json
        or "card_key" not in request.json
        or not request.json["card_key"]
    ):
        db_close(con, cur)
        return jsonify({
            "status": 400,
    ...
    cur.execute("""
        SELECT * FROM card WHERE key = %s;
    """, (request.json["card_key"],))
    card = cur.fetchone()

    if (
        not org or not card
        or user["key"] != org["user_key"]
        or card["organization_key"] != org["key"]
    ):
        db_close(con, cur)
        return jsonify({
            "status": 400,
    ...
    cur.execute("""
        UPDATE card SET status = %s WHERE key = %s;
    """, ("live", card["key"]))
